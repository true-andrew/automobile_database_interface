...
<div class="container-fluid" style="display: flex; flex-direction: column; margin-top: 10px">
    ...
    <table class="table table-bordered table_sort" id="table">
        ...
        <?php foreach ($engines as $engine) { ?>
            <tr>
                <td><?php echo $engine['id_engine']; ?></td>
                <td><?php echo $engine['engine_displacement']; ?></td>
            </tr>
        <?php } ?>
    </table>
    <form method="POST" role="form" id="add">
        <div class="form-group">
            <label>Введите литраж двигателя</label>
            <input type="text" class="form-control" id="name" name="name" placeholder="Введите значение">
        </div>
        <button type="submit" class="btn btn-primary">Добавить</button>
    </form><form class="form2" method="POST" role="form" id="changeRemove" style="display: none">
        <div class="form-group form2">
            <label class="form2">Введите литраж двигателя</label>
    ...
    if(isset($_POST['name']) && $_POST['name'] != '')
    {
        ...
        $name = $_POST['name'];
        writeToСharacteristics($db, 'engine', $name);
    }
    ?>
    <?php
    if(isset($_POST['change']) && $_POST['change'] != '')
    {
        ...
        if (isset($_POST['update_button'])) {
            changeCharacteristics($db, 'engine', $name, $id);
        } else if (isset($_POST['delete_button'])) {
            deleteFromCharacteristics($db, 'engine', $id);
        }
        else {
            var_dump($name);
        }
