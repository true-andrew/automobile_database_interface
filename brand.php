...
<div class="container-fluid" style="display: flex; flex-direction: column; margin-top: 10px">
    ...
    <form class="form2" method="POST" role="form" id="changeRemove" style="display: none">
        <div class="form-group form2">
            <label class="form2">Введите название марки</label>
    ...
    if(isset($_POST['name']) && $_POST['name'] != '')
    {
        ...
        $name = $_POST['name'];
        writeToСharacteristics($db, 'brand', $name);
    }
    ?>
    <?php
    if(isset($_POST['change']) && $_POST['change'] != '')
    {
        ...
        if (isset($_POST['update_button'])) {
            changeCharacteristics($db, 'brand', $name, $id);
        } else if (isset($_POST['delete_button'])) {
            deleteFromCharacteristics($db, 'brand', $id);
        }
        else {
            var_dump($name);
        }
