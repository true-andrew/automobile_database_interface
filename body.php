...
<div class="container-fluid" style="display: flex; flex-direction: column; margin-top: 10px">
    ...
    <table class="table table-bordered table_sort" id="table">
        ...
        <?php foreach ($bodies as $body) { ?>
            <tr>
                <td><?php echo $body['id_body']; ?></td>
                <td><?php echo $body['body_type']; ?></td>
            </tr>
        <?php } ?>
    </table>
    <form method="POST" role="form" id="add">
        <div class="form-group">
            <label>Введите тип кузова</label>
    ...
    <form class="form2" method="POST" role="form" id="changeRemove" style="display: none">
        <div class="form-group form2">
            <label class="form2">Введите тип кузова</label>
    ...
    if (isset($_POST['name']) && $_POST['name'] != '') {
        $name = $_POST['name'];
        writeToСharacteristics($db, 'body', $name);
    }
    ?>
    <?php
    if (isset($_POST['change']) && $_POST['change'] != '') {
        ...
        if (isset($_POST['update_button'])) {
            changeCharacteristics($db, 'body', $name, $id);
        } else if (isset($_POST['delete_button'])) {
            deleteFromCharacteristics($db, 'body', $id);
        } else {
            var_dump($name);
        }
    }
